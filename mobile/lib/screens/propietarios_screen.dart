...
class _PropietariosScreenState extends State<PropietariosScreen> {
  ...
  Future<void> _fetchPropietarios() async {
    ...
    try {
      ...
      setState(() {
        _propietarios = (response.data as List).cast<Map<String, dynamic>>();
        _loading = false;
      });
    } catch (e) {
  ...
  Future<void> _showFormDialog({Map<String, dynamic>? propietario}) async {
    final nombreCtrl = TextEditingController(text: propietario?['nombre'] ?? '');
    final bggCtrl = TextEditingController(text: propietario?['bgg_username'] ?? '');
                  ...
                  : () async {
                      ...
                      try {
                        final data = {
                          ...
                          'bgg_username': bggCtrl.text.trim().isEmpty ? null : bggCtrl.text.trim(),
                        };
                        if (isEditing) {
                          await _api.put('/propietarios/${propietario['id']}', data: data);
                        } else {
                          await _api.post('/propietarios', data: data);
                        }
  ...
  Future<void> _confirmDelete(Map<String, dynamic> propietario) async {
    final esPrincipal = propietario['es_principal'] == true || propietario['es_principal'] == 1;
    if (esPrincipal) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No se puede eliminar el propietario principal')),
      );
      return;
    }
    ...
    if (confirm == true) {
      try {
        await _api.delete('/propietarios/${propietario['id']}');
        _fetchPropietarios();
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 12),
                      Text('No hay propietarios', style: TextStyle(color: Colors.grey[600])),
                    ],
                  ),
                )
              : RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final esPrincipal = prop['es_principal'] == true || prop['es_principal'] == 1;
                      return Card(
                        ...
                        child: ListTile(
                          ...
                          subtitle: prop['bgg_username'] != null && prop['bgg_username'].toString().isNotEmpty
                              ? Text('BGG: ${prop['bgg_username']}',
                                  style: TextStyle(fontSize: 12, color: Colors.grey[600]))
                              : null,
                          trailing: PopupMenuButton<String>(
                            onSelected: (action) {
                              if (action == 'edit') _showFormDialog(propietario: prop);
